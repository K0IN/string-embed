...
DEFAULT_WORDS_PATH = resources.files("string_embed").joinpath("data/words_alpha.txt")
...
def load_words(path: str | Path | None = None, limit: int | None = None, seed: int | None = None) -> list[str]:
    if path is None:
        with DEFAULT_WORDS_PATH.open("r", encoding="utf-8", errors="ignore") as f:
    ...
    indices = sorted(rng.sample(range(len(words)), sample_size))
